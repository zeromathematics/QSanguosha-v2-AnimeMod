...
[":se_jianwu"] = "出牌阶段，你可以弃置130枚“天命”标记并指定至多三名角色，令他们各失去1点体力并将武将牌翻面。",
["se_kanhu"] = "看护「神圣术治疗」",
["kanhu"] = "看护「神圣术治疗」",
			...
			if mygod then
				if mygod:isAlive() and source:getHp() < 1 and mygod:getMark("@HIMIKO") >0 then
					if room:askForSkillInvoke(mygod, "SE_Boming", data) then
						...
						elseif mygod:hasSkill("longdan") then
							i = i + 1
							room:detachSkillFromPlayer(mygod, "longdan")
						elseif mygod:hasSkill("paoxiao") then
							i = i + 1
							room:detachSkillFromPlayer(mygod, "paoxiao")
						elseif mygod:hasSkill("guicai") then
							i = i + 1
...
SE_Boming_KOF = sgs.CreateTriggerSkill{
	name = "#SE_Boming_KOF",
	frequency = sgs.Skill_Compulsory, 
	events = {sgs.GameStart},
	on_trigger = function(self, event, player, data)
		...
		if event == sgs.GameStart then
			if player:hasSkill("SE_Boming") then
				player:loseAllMarks("@HIMIKO")
				player:gainMark("@HIMIKO")
				if room:getAllPlayers(true):length() == 2 then
					room:detachSkillFromPlayer(player, "SE_Boming")
				end
			end
			room:detachSkillFromPlayer(player, self:objectName())
		end
...
[":se_shouren"] = "<font color=\"green\"><b>出牌阶段限一次，</b></font>你可以指定一名角色，令其失去1点体力。若如此做且该角色的当前体力值为0，你令其将武将牌翻面。",
["Kanade"] = "立华奏", 
["&Kanade"] = "立华奏", 
["@Kanade"] = "Angel Beats!", 
["#Kanade"] = "天使", 
...
SE_Chengzhang = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		room:acquireSkill(player, "se_zhiling")
		...
		if target then
			if target:isAlive() and target:hasSkill(self:objectName()) then
				if target:getPhase() == sgs.Player_Start then
					if target:getMark("@waked") == 0 then
						local counts = target:getPile("Neko"):length()
						return counts >= 12
					end
				end
			end
...
se_zhiling = sgs.CreateViewAsSkill{
	name = "se_zhiling", 
	n = 0,
	view_as = function(self, cards)
		return se_zhilingcard:clone()
	end, 
	enabled_at_play = function(self, player)
...
se_zhilingcard = sgs.CreateSkillCard{
	...
	will_throw = true,
	filter = function(self, targets, to_select)
		return #targets == 0 and (to_select:getMark("@Neko_S") == 0 or to_select:getMark("@Neko_C") == 0 or to_select:getMark("@Neko_D") == 0 or to_select:getMark("@Neko_H") == 0) and not to_select:hasFlag("Can_not")
	end,
	on_use = function(self, room, source, targets)
		local target = targets[1]
		local Neko_all = source:getPile("Neko")
		...
		local count = Neko:length()
		local id
		if count < 1 then
			room:setPlayerFlag(target, "Can_not")
			return
		else
			room:fillAG(Neko, source)
			id = room:askForAG(source, Neko, false, self:objectName())
			room:clearAG(source)
			if id == -1 then
				return
			end
...
SE_Zhixing = sgs.CreateTriggerSkill{
	name = "SE_Zhixing", 
	frequency = sgs.Skill_NotFrequent, 
	events = {sgs.Dying,sgs.EventLoseSkill},
	on_trigger = function(self, event, player, data)
		if event == sgs.Dying then
			local room = player:getRoom()
			local dying_data = data:toDying()
			...
			if mygod then
				if mygod:isAlive() and source and not mygod:isKongcheng() then
					...
					if room:askForSkillInvoke(mygod, "SE_Zhixing", data) then
						...
						local target = room:askForPlayerChosen(mygod, targets, self:objectName())
...
SE_Yirong = sgs.CreateTriggerSkill{
	...
	on_trigger = function(self, event, player, data)
		...
		elseif event == sgs.EventPhaseStart then
			local phase = player:getPhase()
			--KOF
			if (phase == sgs.Player_RoundStart and room:getAllPlayers(true):length() > 2) or phase == sgs.Player_Play or phase == sgs.Player_Discard  then
				if room:askForSkillInvoke(player, self:objectName()) then
...
[":se_tongling"] = "<font color=\"red\"><b>限定技，</b></font>出牌阶段，你可以选择一项：1、指定一名主公以外的其他角色，你杀死该角色，然后令其复活并变身为僵尸。2、指定一名已死亡角色，令其变身为僵尸。",
["Eucliwood"] = "优克莉伍德", 
["&Eucliwood"] = "优克莉伍德", 
["@Eucliwood"] = "这就是僵尸吗", 
["#Eucliwood"] = "沉默の死灵师", 
